...
class ViewController: UIViewController, RabbitNameEditViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(ViewController.updateUI(_:)), name: "UpdateUINotify", object: nil)
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "EditRabitName" {
            ...
            self.addObserver(self, forKeyPath: "rabbitName", options: .New, context: nil)
            ...
            // Match slide 50
            self.rabbitName = self.rabbitNameLabel.text
            controller.rabbitName = self.rabbitName
            

            // Closure listen
            controller.editHandler = {(editName) -> Void in
                // Call back later
